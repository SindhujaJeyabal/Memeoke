...
def showLyrics():
	...
	return flask.render_template('demo.html', lyrics=json.dumps(time_words), gifs = gifs)

@app.route('/artists.html')
def mm_topartists():
	artists = lrc.artist_search()
	return flask.render_template('data_panel.html', subheader_val='0', track_id='0', artists = artists, gifs=list(), soundcloud_id='-1')

@app.route('/tracks.html/<artist_id>/<artist_name>')
def mm_toptracks(artist_id, artist_name):
	tracks = lrc.top_tracks(artist_id)
	return flask.render_template('data_panel.html', subheader_val='1', track_id='0', artist_name = artist_name, gifs = list(), tracks = tracks, soundcloud_id='-1')
...
def mm_tracklyrics(track_id, artist_name, track_name, soundcloud_id):
	...
	return flask.render_template('data_panel.html', subheader_val='2', artist_name = artist_name, track_id=track_id, lyrics = lyrics, gifs = gifs, track_name=track_name, soundcloud_id = soundcloud_id)
